...
static int udev_fd;
static struct input_config inputConfig;
  ...
  if (action != NULL) {
    if (autoadd && strcmp("add", action) == 0) {
      ...
      if (devnode != NULL && sscanf(devnode, "/dev/input/event%d", &id) == 1) {
        evdev_create(devnode, defaultMappings, debug, &inputConfig);
      }
    }
...
void udev_init(bool autoload, struct mapping* mappings, bool verbose, struct input_config* config) {
  udev = udev_new();
  debug = verbose;
  if (!udev) {
  ...
  if (autoload) {
    ...
    udev_list_entry_foreach(dev_list_entry, devices) {
      ...
      int id;
      if (devnode != NULL && sscanf(devnode, "/dev/input/event%d", &id) == 1) {
        evdev_create(devnode, mappings, verbose, config);
      }
      udev_device_unref(dev);
    }
